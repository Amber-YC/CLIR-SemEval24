...
class BaselineNN(nn.Module):
    ...
    def evaluate(self, eval_data):
        self.eval()
        total_loss = 0.0
        eval_input = get_batches(batch_size=batch_size, data=eval_data)
        batch_num = len(eval_input)

        with torch.no_grad():
            for batch in tqdm(eval_input, total=batch_num, desc="Evaluation", ncols=80):
...
def train_model(train_data, test_data, epochs=epochs, opt=opt):
    ...
    best_model = BaselineNN(transformer_model=bertmodel.load_state_dict(loaded_model_state_dict))
